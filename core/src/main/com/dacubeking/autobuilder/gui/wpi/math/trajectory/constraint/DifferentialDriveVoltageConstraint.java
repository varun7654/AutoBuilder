...
/**
 * A class that enforces constraints on differential drive voltage expenditure based on the motor dynamics and the drive
 * kinematics. Ensures that the acceleration of any wheel of the robot while following the trajectory is never higher than what
 * can be achieved with the given maximum voltage.
 */
@Constraint(name = "Differential Drive Voltage", description = """
        A class that enforces constraints on differential drive voltage expenditure based on the motor dynamics and the drive kinematics.\s
...
public class DifferentialDriveVoltageConstraint implements TrajectoryConstraint {
    ...
    @ConstraintField(name = "Kinematics", description = "The kinematics for the robot drivetrain.")
     ...
     * @param maxVoltage  The maximum voltage available to the motors while following the path. Should be somewhat less than the
     *                    nominal battery voltage (12V) to account for "voltage sag" due to current draw.
     */
    @JsonCreator
    ...
    public TrajectoryConstraint copy() {
        return new DifferentialDriveVoltageConstraint(m_feedforward.copy(), m_kinematics.copy(), m_maxVoltage);
    }
}
